...
> **Version:** v1.2.2 | **Last Updated:** 2026-06-12
...
> 💡 **ビルド済みexeを直接ダウンロードしたい場合**は、GitHub Actions による自動ビルドを利用できます（下記「ビルド済みexeの入手」を参照）。手元でビルドする必要はありません。
...
### 10. **.github/workflows/build-exe.yml** （exe自動ビルド）
GitHub Actions で `PDF2OCR.exe` を自動ビルドするワークフロー定義

**動作:**
- `v*` タグを push すると、Windowsランナーでexeをビルドし **Releases** に自動添付
- 手動実行（Actions タブ）すると、ビルドしたexeを **Artifact** として保存

**推奨用途:** 手元でビルドせずに、ビルド済みexeを配布・入手したいとき（詳細は下記「ビルド済みexeの入手」）
...
## 📥 ビルド済みexeの入手（GitHub Actions 自動ビルド）

`PDF2OCR.exe` は GitHub Actions（Windowsランナー）で自動ビルドされます。
手元でビルドしなくても、以下の方法でダウンロードできます。

### 方法A: Releases から入手（推奨）

バージョンタグ（`v*`）が付くと、自動でビルドされ Releases ページに添付されます。

1. リポジトリの **Releases** ページを開く
   ```
   https://github.com/quasrg-design/PDF2OCR/releases
   ```
2. 最新リリースの **Assets** から `PDF2OCR.exe` をダウンロード

> リリースを発行するには、メンテナがタグを push します:
> ```bash
> git tag v1.2.1
> git push origin v1.2.1
> ```

### 方法B: Actions の成果物（Artifact）から入手

1. リポジトリの **Actions** タブを開く
2. **Build Windows EXE** ワークフローを選択し、「Run workflow」で手動実行（または最新の成功した実行を開く）
3. 実行結果ページ下部の **Artifacts** から `PDF2OCR-exe` をダウンロード

> ⚠️ いずれの場合も、exeを実行するPCには **Tesseract-OCR（日本語パック付き）** と **Poppler** のインストールが必要です。
